...
def _(inpaint, np):
    ...
    def inpaint_to_height(img):
        ...
        return height_mm
    return (inpaint_to_height,)


@app.cell
def _(ConvexHull, Path, inpaint_to_height, np, skimage):
    ...
    return (SingleTuberImgPair,)


@app.cell
def _(SingleTuberImgPair, foo):
    bar = SingleTuberImgPair(foo.yolo_res[11])
    bar.calc_cls_proportions()
    bar.mask_fg()
    bar.calc_tuber_size()
    print(bar.tuber_metrics)
    return (bar,)


@app.cell
def _(Path, SingleTuberImgPair, pd):
    class TuberImgSet():
